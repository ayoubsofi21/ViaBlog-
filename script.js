  ...
  array.forEach((trip) => {
    ...
    card.innerHTML = `
      ...
      <div class="imghome mt-4">
        <a href="details.html">
          <img src="${trip.image || "images/imgstory.webp"}" class="imgTrip rounded-t-[15px] w-[100%] h-70" alt="image-home">
        </a>
      </div>
      <div class="localClockkm flex justify-between m-2">
        ...
        <div class="relative">
          ...
          <div class="menu hidden absolute right-0 top-6 bg-gray-700 text-white rounded shadow-md w-28">
              <button class="updateBtn block w-full text-left px-3 py-2 hover:bg-gray-600">
              Update
              </button>
              <button class="deleteBtn block w-full text-left px-3 py-2 hover:bg-red-600">
              Delete
              </button>
                  </div>
        </div>
    ...
    deleteBtn.addEventListener("click", () => {
      const index = array.findIndex((t) => t.id === trip.id);
...
form.addEventListener("submit", function (e) {
  ...
  // attachEvents();
  if (editId) {
    const trip = array.find((t) => t.id === editId);
    trip.Title = title;
    trip.date = date;
    trip.image = image;
    trip.note = note;
